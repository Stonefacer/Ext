...
namespace Ext.System.Security {
    ...
    #region Contructors
        ...
        public static advRSA Instance(BigInteger P, BigInteger Q, BigInteger E, BigInteger D) {
            advRSA buf = new advRSA();
            buf.P = P;
            buf.Q = Q;
            buf.E = E;
        ...
        public static advRSA Instance(byte[] P, byte[] Q, byte[] E, byte[] D) {
            advRSA buf = new advRSA();
            buf.P = new BigInteger(P);
            buf.Q = new BigInteger(Q);
            buf.E = new BigInteger(E);
    ...
    #region Static
        ...
        public BigInteger[] Encrypt<T>(T lst) where T : IEnumerable, ICollection {
        ...
        public BigInteger[] Decrypt<T>(T lst) where T : IEnumerable, ICollection {
            BigInteger[] bi = new BigInteger[lst.Count];
            int i = 0;
            foreach(BigInteger buf in lst) {
                bi[i++] = Decrypt(buf);
        ...
        public bool CheckSignature(byte[] signature, byte[] hash) {
            ...
            var res = Encrypt(data).ToByteArray();
